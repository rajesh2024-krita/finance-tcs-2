  ...
  @Component({
    ...
    <div class="bg-white border">
      <div class="flex justify-between items-center p-6 bg-gradient-to-br from-indigo-400 to-purple-600 text-white">
        <div>  
          <h2 class="text-2xl font-semibold mb-4">Loan Entry Form</h2>
        </div>
        <div class="space-x-2">
          <button (click)="onValidate()" class="px-3 py-1 rounded bg-[#FF8040] text-white">Validate</button>
          <button (click)="onSave()" [disabled]="!canSave" class="px-3 py-1 rounded bg-[#0046FF] text-white">Save</button>
      ...
      <form [formGroup]="form" (ngSubmit)="onSave()" class="space-y-4 p-6">
        <div>
            <label class="block text-xs font-medium">Loan Type</label>
        ...
        <div class="grid md:grid-cols-2 grid-cols-1 gap-4">
          <div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border p-3 mb-2">
              <div>
                <label class="block text-xs font-medium">Loan No.</label>
              ...
              <div>
                <label class="block text-xs font-medium">Loan Date</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              ...
              <div>
                <label class="block text-xs font-medium">EDP No.</label>
              ...
              <div>
                <label class="block text-xs font-medium">Name (member)</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div>
                <label class="block text-xs font-medium">Loan Amount (₹)</label>
                <input type="number" formControlName="loanAmount" (input)="recalculate()" class="mt-1 p-2 border rounded w-full" />
              </div>

              
              <div>
                <label class="block text-xs font-medium">Previous Loan (Remaining)</label>
            ...
            <div class="grid md:grid-cols-2 grid-cols-1 gap-4 mt-4">
              <div class="space-y-4"> 
                <div>
                  <label class="block text-xs font-medium">Net Loan (Auto)</label>
                  <input [value]="formatCurrency(netLoan())" readonly class="mt-1 p-2 border rounded w-full bg-slate-50" />
                </div>

                <div>
                  <label class="block text-xs font-medium">No. of Installments</label>
                  <input type="number" readonly formControlName="installments" (input)="recalculate()" class="mt-1 p-2 border rounded w-full" />
                </div>
                <div>
                  <label class="block text-xs font-medium">Installment Amount (Auto)</label>
                  <input [value]="formatCurrency(installmentAmount())" readonly class="mt-1 p-2 border rounded w-full bg-slate-50" />
                </div>

                <div class="sr-only">
                  <label class="block text-xs font-medium">Interest Rate (Auto)</label>
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <textarea formControlName="purpose" rows="4" class="mt-1 p-2 border rounded w-full"></textarea>
                </div>
                <div class="">
                  <label class="block text-xs font-medium">Authorized By <span class="text-[11px]">(In the case of Limit Exceed)</span></label>
                  <input formControlName="authorizedBy" class="mt-1 p-2 border rounded w-full" />
                </div>
              </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-1 gap-4 border p-4 mt-4">
              <div>
                <label class="block text-xs font-medium">Payment Mode</label>
          ...
          <div>
            ...
            <div class="border-t pt-4">
              <h3 class="font-semibold mb-2">Surety Tables</h3>
              <div>
                <div class="flex border-b mb-2">
                  <button type="button" (click)="activeTab='given'" [class.border-b-2]="activeTab==='given'" [class.font-bold]="activeTab==='given'" class="px-4 py-2">Surety Given</button>
                  <button type="button" (click)="activeTab='taken'" [class.border-b-2]="activeTab==='taken'" [class.font-bold]="activeTab==='taken'" class="px-4 py-2">Surety Taken</button>
                </div>
                <div *ngIf="activeTab==='given'">
                  <table class="w-full mt-2 border">
                    <thead class="bg-slate-50"><tr><th class="p-2">MemNo</th><th class="p-2">Name</th></tr></thead>
                    <tbody>
                      <tr *ngFor="let s of givenSurety"><td class="p-2">{{s.memNo}}</td><td class="p-2">{{s.name}}</td></tr>
                    </tbody>
                  </table>
                </div>
                <div *ngIf="activeTab==='taken'">
                  <table class="w-full mt-2 border">
                    <thead class="bg-slate-50"><tr><th class="p-2">MemNo</th><th class="p-2">Name</th></tr></thead>
                    <tbody>
                      <tr *ngFor="let s of takenSurety"><td class="p-2">{{s.memNo}}</td><td class="p-2">{{s.name}}</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
  ...
  if (installments > 0) {
    ...
    this.form.patchValue({ installmentAmount: emi }, { emitEvent: false });
  }
}
